...
- **Files** — the workspace as it actually is on disk: a tree with git status beside a reader that renders markdown, PDFs, images and code. While a run is in flight the agent writes into its own working copy, so the panel lets you switch between your project folder and that copy — the answer to "the agent said it wrote four files, my folder looks empty".
