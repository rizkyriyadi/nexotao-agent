...
function Row({ href, title, meta, capitalizeMeta = false }: { href: string; title: string; meta: string; capitalizeMeta?: boolean }) { return <Link href={href} className="block rounded-xl border border-line bg-raise/60 p-3 hover:border-line-strong"><p className="truncate text-xs font-medium text-charcoal">{title}</p><p className={`mt-1 truncate text-[11px] text-pebble${capitalizeMeta ? " capitalize" : ""}`} title={meta}>{meta}</p></Link>; }
function Risk({ value }: { value: string | null }) { const tone = value === "high" ? "bg-alarm-red/10 text-alarm-red" : "bg-amber/12 text-amber"; return <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${tone}`}>{value || "medium"}</span>; }
