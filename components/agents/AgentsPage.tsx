...
// `capitalize` only for enum-ish values (status). Free text — a task title or a
// heartbeat phrase — must render verbatim; see the note in components/inbox/Inbox.
function Metric({ label, value, capitalizeValue = false }: { label: string; value: string; capitalizeValue?: boolean }) { return <div className="rounded-2xl border border-line bg-paper-white p-4"><p className="label">{label}</p><p className={`mt-2 truncate text-[15px] font-medium${capitalizeValue ? " capitalize" : ""}`} title={value}>{value}</p></div>; }
